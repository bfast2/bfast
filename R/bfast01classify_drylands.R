...
# Return: integer representing the class and subclass
## classification function for 1-break bfast output, optmized for drylands
## object: bfast01 object
## alpha: threshold for significance tests, default 0.05

bfast01classify_drylands <- function(object, alpha=0.05) {
  ## output array
  out <- rep(NA,7)
  names(out) <- c("flag_type","flag_subtype","flag_significance",
                  "p_segment1","p_segment2","pct_segment1","pct_segment2")
  ## data series
  object.zoo <- as.zoo(object)
  ...
  for (segment in 1:(object$breaks+1)) {
    ...
    out[segment+3] <- segment.anova$Pr[1]
    # PctChange
    ...
    out[segment+5] <- segment.pctchange
  }
  
  p1 <- out[4] #significance of the first (or only) trend
  p2 <- out[5] #significance of the second trend
  ...
  if(object$breaks == 0) {
    ...
    if(slope > 0) out[1] <- 1 # mono inc
    if(slope < 0) out[1] <- 2 # mono dec
    }
  } else {
    ## classes with break
  ...
  if(object$breaks != 0 && out[1] != 0) { #if there's a break and at least one sign. trend
    ...
    if( (p1 <= alpha && p2 > alpha) || (p1 > alpha && p2 <= alpha) ) out[2] <- 3 # transition
    if(p1 <= alpha && p2 <= alpha) out[2] <- 4 # complete
    }
  }
  ...
  # no break
  if(object$breaks == 0) {
    if(out[4] <= alpha) out[3] <- 0
    if(out[4] > alpha) out[3] <- 3
    # with break
  } else {
    if(out[4] <= alpha && out[5] <= alpha) out[3] <- 0
    if(out[4] <= alpha && out[5] > alpha) out[3] <- 1
    if(out[4] > alpha && out[5] <= alpha) out[3] <- 2
    if(out[4] > alpha && out[5] > alpha) out[3] <- 3
  }
